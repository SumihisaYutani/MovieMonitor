...
public class MainViewModel : ObservableObject
{
    ...
    // ランダム再生コマンド
    [RelayCommand] private void CreateRandomPlaylist();
    [RelayCommand] private async Task PlayNextVideoAsync();
    [RelayCommand] private async Task PlayPreviousVideoAsync();
    [RelayCommand] private void StopRandomPlay();
    
    // ランダム再生プロパティ
    [ObservableProperty] private bool _isRandomPlayModeEnabled;
...
<Grid>
  ...
  <StackPanel Grid.Row="0" Style="{StaticResource SearchPanelStyle}">
    ...
    <!-- ランダム再生コントロール -->
    <Button Content="ランダム再生" Command="{Binding CreateRandomPlaylistCommand}"/>
    <Button Content="◀" Command="{Binding PreviousVideoCommand}" 
            IsEnabled="{Binding IsRandomPlayModeEnabled}"/>
    <Button Content="▶" Command="{Binding NextVideoCommand}" 
            IsEnabled="{Binding IsRandomPlayModeEnabled}"/>
    <Button Content="⏹" Command="{Binding StopRandomPlayCommand}" 
            IsEnabled="{Binding IsRandomPlayModeEnabled}"/>
  </StackPanel>
...
7. ウィンドウを最大化状態で起動（WindowState="Maximized"）
8. 前回スキャンデータの読み込み・UI 更新
9. FFmpeg パス検証・サムネイル生成準備
10. 初期表示完了・ユーザー操作待機
```
...
### 5.4 ランダム再生フロー
```
1. ユーザーがランダム再生ボタンクリック
2. CreateRandomPlaylistCommand 実行
3. 現在表示中の FilteredVideos からランダムプレイリスト作成
4. リストをシャッフル（Random.Next()使用）
5. IsRandomPlayModeEnabled = true でUI状態更新
6. 再生制御ボタン（前/次/停止）アクティブ化
7. 次/前ボタンでプレイリスト内移動・動画再生
8. 停止ボタンでプレイリストクリア・UI状態リセット
9. StatusText でプレイリスト状況表示（「再生中: ファイル名 (N/M件)」）
```
